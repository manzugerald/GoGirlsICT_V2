...
export default function HeroSection({ content }: HeroSectionProps) {
  ...
  if (!content?.heroVideo?.trim()) {
    return (
      <div className="w-full h-96 bg-gradient-to-br from-pink-500 to-purple-600 flex items-center justify-center">
  ...
  return (
    <div className="hero-section relative w-full h-screen overflow-hidden">
      ...
      <div className="relative z-10 h-full flex flex-col items-center justify-center space-y-8">
        {/* Site name/logo at top */}
        <div className="text-center mb-8 px-4">
          <h1 className="text-4xl md:text-6xl lg:text-7xl font-bold text-white drop-shadow-2xl mb-4">
            {content.siteName || 'GoGirls ICT Initiative'}
          </h1>
          <div className="h-1 w-32 bg-pink-500 mx-auto rounded-full" />
        </div>

        {/* Scrolling text cards container - FULL WIDTH */}
          ...
          <div className="space-y-4">
            ...
            <div className={`flex gap-4 ${mounted ? 'animate-scroll-left' : ''}`}>
              {[...TEXT_SEGMENTS, ...TEXT_SEGMENTS].map((text, idx) => (
                <div
                  key={`row1-${idx}`}
                  className="flex-shrink-0 bg-white/10 backdrop-blur-md border border-white/20 rounded-lg px-6 py-4 min-w-[300px] max-w-[400px]"
                >
            ...
            <div className={`flex gap-4 ${mounted ? 'animate-scroll-right' : ''}`}>
              {[...TEXT_SEGMENTS, ...TEXT_SEGMENTS].map((text, idx) => (
                <div
                  key={`row2-${idx}`}
                  className="flex-shrink-0 bg-pink-600/20 backdrop-blur-md border border-pink-400/30 rounded-lg px-6 py-4 min-w-[300px] max-w-[400px]"
                >
                  <p className="text-white text-sm md:text-base font-medium leading-relaxed">
                    {text}
                  </p>
        ...
        <div className="mt-8 flex flex-wrap gap-4 justify-center px-4">
          <a
            href="#our-work"
            className="px-8 py-3 bg-pink-600 hover:bg-pink-700 text-white font-semibold rounded-full transition-all shadow-lg hover:shadow-xl"
          >
            Explore Our Work
          </a>
          <a
            href="#get-involved"
            className="px-8 py-3 bg-white/10 hover:bg-white/20 backdrop-blur-md text-white font-semibold rounded-full border border-white/30 transition-all"
          >
            Get Involved
          </a>
        </div>
