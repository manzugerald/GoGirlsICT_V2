...
"[project]/.next-internal/server/app/api/facebook-posts/route/actions.js [app-rsc] (server actions loader, ecmascript)": (function(__turbopack_context__) {
...
"[project]/app/api/facebook-posts/route.ts [app-route] (ecmascript)": ((__turbopack_context__) => {
"use strict";
...
async function GET(req) {
    ...
    const redisCached = await __TURBOPACK__imported__module__$5b$project$5d2f$utils$2f$redis$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["redis"].get(REDIS_CACHE_KEY);
    if (redisCached) {
        const cachedData = JSON.parse(redisCached);
        return Response.json(cachedData);
    }
    let meta = await prisma.facebookCacheMeta.findUnique({
        where: {
            id: 1
        }
    });
    let needsFetch = true;
    if (meta && meta.lastFetched && (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$dayjs$2f$dayjs$2e$min$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["default"])().diff((0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$dayjs$2f$dayjs$2e$min$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["default"])(meta.lastFetched), 'hour') < 24) needsFetch = false;
    if (!needsFetch) {
        ...
        // cache in Redis
        await __TURBOPACK__imported__module__$5b$project$5d2f$utils$2f$redis$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["redis"].set(REDIS_CACHE_KEY, JSON.stringify(result), 'EX', REDIS_CACHE_TTL);
        return Response.json(result);
    }
    const fbRes = await fetch(`https://graph.facebook.com/v19.0/${PAGE_ID}/posts?fields=id,message,created_time,permalink_url,full_picture&access_token=${ACCESS_TOKEN}`);
    ...
    const toSave = (fbData.data || [])// Test: temporarily remove this filter if you get zero results
    .filter((p)=>p.full_picture).map((p)=>({
            id: p.id,
    ...
    if (toSave.length > 0) {
        try {
            const result = await prisma.facebookPost.createMany({
                data: toSave
            });
    ...
    const postsInDb = await prisma.facebookPost.findMany({
        orderBy: {
            createdTime: 'desc'
        }
    });
...
//# sourceMappingURL=%5Broot-of-the-server%5D__455efe43._.js.map