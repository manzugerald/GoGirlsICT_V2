...
onOpenEdit }) {
    ...
    function openEdit(forPassword = false, forUsername = false) {
        ...
        if ("TURBOPACK compile-time truthy", 1) {
            const userIdToUse = id ?? sessionUserId;
            if (!userIdToUse) return;
            const url = new URL(window.location.href);
            url.searchParams.set('type', 'users');
            url.searchParams.set('editUserId', String(userIdToUse));
            window.history.replaceState({}, '', url.toString());
            // You can also trigger a custom event if your dashboard listens for it.
            window.dispatchEvent(new CustomEvent('open-user-edit', {
                detail: {
                    id: userIdToUse,
                    forceChangePassword: forPassword,
                    forceChangeUsername: forUsername
...
_s(SettingsSection, "ujwIunAD3hlHFoJLG3BNiDLiMqM=", false, function() {
    return [
...
//# sourceMappingURL=_1f49bc41._.js.map